...
add_action(
	...
	<div class="form-field">
		<label for="_max_installments"><?php esc_html_e( 'MercadoPago: Maxima cantidad de cuotas para esta categoría', 'wc-kmercadopago-gpl' ); ?></label>
		<input type="text" disabled>
		<?php echo esc_html( __( 'Only available in MercadoPago Tools Pro', 'wc-kmercadopago-gpl' ) ); ?>: <a href="https://yipi.app/p/mercadopago-para-woocommerce/?utm_source=wordpress_gpl&utm_medium=wordpress_gpl&utm_campaign=wordpress_gpl&utm_id=wordpress_gpl&utm_term=wordpress_gpl&utm_content=wordpress_gpl" target="_blank">https://yipi.app/</a>
	</div>
		<?php
		$installments = array( 3, 6, 9, 12, 18 );
		foreach ( $installments as $installment ) {
			?>
			<div class="form-field">
				<label for="_fee_installments_msi_<?php echo esc_attr( $installment ); ?>"><?php echo esc_html( sprintf( __( 'MercadoPago: Porcentaje de Comisión para %s Meses sin Intereses (Solo para Basic Checkout)', 'wc-kmercadopago-gpl' ), $installment ) ); ?></label>
				<input type="text" disabled>
				<?php echo esc_html( __( 'Only available in MercadoPago Tools Pro', 'wc-kmercadopago-gpl' ) ); ?>: <a href="https://yipi.app/p/mercadopago-para-woocommerce/?utm_source=wordpress_gpl&utm_medium=wordpress_gpl&utm_campaign=wordpress_gpl&utm_id=wordpress_gpl&utm_term=wordpress_gpl&utm_content=wordpress_gpl" target="_blank">https://yipi.app/</a>
			</div>
			<?php
...
add_action(
	...
	function ( $term ) {
		?>
	<tr class="form-field">
		<th scope="row" valign="top"><label for="_max_installments"><?php esc_html_e( 'MercadoPago: Maxima cantidad de cuotas para esta categoría', 'wc-kmercadopago-gpl' ); ?></label></th>
		<td>
			<input type="text" disabled>
			<?php echo esc_html( __( 'Only available in MercadoPago Tools Pro', 'wc-kmercadopago-gpl' ) ); ?>: <a href="https://yipi.app/p/mercadopago-para-woocommerce/?utm_source=wordpress_gpl&utm_medium=wordpress_gpl&utm_campaign=wordpress_gpl&utm_id=wordpress_gpl&utm_term=wordpress_gpl&utm_content=wordpress_gpl" target="_blank">https://yipi.app/</a>
		</td>
		...
		foreach ( $installments as $installment ) {
			?>
			<tr class="form-field">
				<th scope="row" valign="top"><label for="_fee_installments_msi_<?php echo esc_attr( $installment ); ?>">
				<?php
				// translators: %s: Installments.
				echo esc_html( sprintf( __( 'MercadoPago: Porcentaje de Comisión para %s Meses sin Intereses (Solo para Basic Checkout)', 'wc-kmercadopago-gpl' ), $installment ) );
				?>
				</label></th>
				<td>
					<input type="text" disabled>
...
add_filter(
	...
	function ( $tabs ) {
		$tabs['mercado_pago_tab_gpl']       = array(
			'label'    => __( 'MercadoPago', 'wc-kmercadopago-gpl' ),
			'target'   => 'mercado_pago_tab_gpl',
			'priority' => 100,
		);
		return $tabs;
	}
...
add_action(
	...
	function () {
		echo '<div id="mercado_pago_tab_gpl" class="panel woocommerce_options_panel">';
		echo esc_html( __( 'Only available in MercadoPago Tools Pro', 'wc-kmercadopago-gpl' ) ) . ': <a href="https://yipi.app/p/mercadopago-para-woocommerce/?utm_source=wordpress_gpl&utm_medium=wordpress_gpl&utm_campaign=wordpress_gpl&utm_id=wordpress_gpl&utm_term=wordpress_gpl&utm_content=wordpress_gpl" target="_blank">https://yipi.app/</a><br />';
		woocommerce_wp_text_input(
			array(
				'id'                => '_max_installments_gpl',
				'label'             => __( 'Maxima cantidad de cuotas para este producto', 'wc-kmercadopago-gpl' ),
				'desc_tip'          => 'true',
				'custom_attributes' => array(
					'disabled' => 'disabled',
				),
				'description'       => __( 'Maxima cantidad de cuotas', 'wc-kmercadopago-gpl' ),
			)
		);
		$installments = array( 3, 6, 9, 12, 18 );
		foreach ( $installments as $installment ) {
			woocommerce_wp_text_input(
				array(
					'id'                => '_fee_installments_msi_gpl_' . $installment,
					// translators: %s is the number of installments.
					'label'             => sprintf( __( 'Porcentaje de Comisión para %s Meses sin Intereses', 'wc-kmercadopago-gpl' ), $installment ),
					'desc_tip'          => 'true',
					'custom_attributes' => array(
						'disabled' => 'disabled',
					),
					'description'       => __( 'Dejar vacío para si no tienes Meses sin Intereses activo o no te interesa cobrar comisión', 'wc-kmercadopago-gpl' ),
				)
			);
		}
		echo '</div>';
