	...
	class WC_KMercadoPagoGPL_Manager extends WC_Integration {
		...
		public function status_changed( $order_id, $old_status = false, $new_status = false ) {
			...
			foreach ( explode( ',', $all_op_id ) as $mp_op_id ) {
				...
				$data     = self::mp_refund_payment( $mp_op_id, $token );
				if ( $data ) {
					self::set_metadata( $order_id, 'is_refunded', true );
				}
				$data = self::mp_cancel_payment( $mp_op_id, $token );
				if ( $data ) {
					self::set_metadata( $order_id, 'is_refunded', true );
				}
			}
		...
		public static function debug( $message, $data = false ) {
			...
			if ( self::$show_debug && ! empty( $message ) ) {
				...
				$context = array( 'source' => 'wc-kmercadopago-gpl' );
				$logger->debug( $message . ( $data ? ' DATA->#' . self::pL( $data ) . '#' : '' ), $context );
			}
		}
		 ...
		 * @return array
		 */
		private static function mp_refund_payment( $mp_op_id, $token = false ) {
			$url      = self::$api_url . "/v1/payments/$mp_op_id/refunds?access_token=" . ( $token ? $token : self::get_client_credentials() );
			$params   = array(
				'sslverify' => false,
				'timeout'   => 60,
				'headers'   => array(
			...
			if ( ! is_wp_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 ) {
				$body = json_decode( $response['body'], true );
				self::debug( 'mp_refound_payment valido: ' . self::pL( $body, true ) );
				return $body;
			} else {
				self::debug( 'mp_refound_payment invalido: ' . self::pL( $url, true ) . self::pL( $response, true ) );
			}
			return false;
		}

		/**
		 * Cancel payment of MercadoPago
		 *
		 * @param string $mp_op_id Payment ID.
		 * @param string $token Access Token.
		 *
		 * @return array
		 */
		private static function mp_cancel_payment( $mp_op_id, $token = false ) {
			...
			if ( ! is_wp_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 ) {
				$body = json_decode( $response['body'], true );
				self::debug( 'mp_cancel_payment valido: ' . self::pL( $body, true ) );
				return $body;
			} else {
				self::debug( 'mp_cancel_payment invalido: ' . self::pL( $url, true ) . self::pL( $response, true ) );
			}
			return false;
		}
